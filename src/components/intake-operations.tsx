...
type PageResult = { items: Item[]; total: number; page: number; limit: number; hasNext?: boolean };
type ConversationDetail = { conversation: Item & { session?: Item | null; ticketLinks?: Item[] }; messages: PageResult; attachments: PageResult };
...
export function IntakeOperations({ diagnosticsAvailable }: { diagnosticsAvailable: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    <Dialog open={Boolean(detail)} onOpenChange={(open) => !open && setDetail(undefined)}><DialogContent title="Sanitized conversation detail" description="Plain-text previews and safe attachment metadata only." className="max-w-4xl"><div className="max-h-[65vh] space-y-4 overflow-y-auto pr-1 text-[11px]">
      ...
      <section><p className="mb-2 font-semibold">Messages ({detail?.messages.total || 0})</p><div className="space-y-2">{detail?.messages.items.map((message) => <div key={text(message.messageId)} className="rounded-xl border border-sky-100 bg-white/65 p-3 dark:border-slate-700 dark:bg-slate-900/60"><div className="flex justify-between gap-3"><Badge tone="blue">{text(message.direction)}</Badge><span className="text-slate-400">{timestamp(message.receivedAt)}</span></div><p className="mt-2 whitespace-pre-wrap leading-5 text-slate-600 dark:text-slate-300">{text(message.textPreview, "No plain-text content")}</p></div>) || <p className="text-slate-400">No messages.</p>}{detail?.messages.hasNext && <p className="text-slate-400">More messages are available on the next bounded page.</p>}</div></section>
      <section><p className="mb-2 font-semibold">Attachment metadata ({detail?.attachments.total || 0})</p><div className="grid gap-2 sm:grid-cols-2">{detail?.attachments.items.map((attachment) => <div key={text(attachment.attachmentId)} className="rounded-xl border border-sky-100 p-3 dark:border-slate-700"><p className="font-semibold">{text(attachment.fileName)}</p><p className="mt-1 text-slate-400">{text(attachment.contentType)} · {number(attachment.declaredSize)} bytes</p><p className="mt-1">{text(attachment.storageStatus)} / {text(attachment.scanStatus)}</p></div>) || <p className="text-slate-400">No attachment metadata.</p>}{detail?.attachments.hasNext && <p className="text-slate-400">More attachment metadata is available on the next bounded page.</p>}</div></section>
