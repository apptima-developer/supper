...
export function ServiceNowWriteControls() {
  ...
  const [sourceReference, setSourceReference] = useState(`manual:${composerId}`);
  ...
  const [confirmExecute, setConfirmExecute] = useState<ServiceNowWriteCommandSummary>();
  ...
  async function commandAction(command: ServiceNowWriteCommandSummary, action: "dry-run" | "execute" | "retry") {
    setBusy(action);
    setConfirmExecute(undefined);
    try {
      const updated = await api<ServiceNowWriteCommandSummary>(
        `/api/integrations/servicenow/write/commands/${encodeURIComponent(command.id)}/${action}`,
        { method: "POST" },
      );
      setSelected(updated);
  ...
  async function testReadiness() {
    setBusy("readiness");
    try {
      await api("/api/integrations/servicenow/write/readiness", { method: "POST" });
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <Stat label="Write readiness" value={summary?.readiness.ready ? "Ready" : "Blocked"} detail={summary?.readiness.safeErrorMessage || summary?.readiness.hostname || "Loading"} />
    ...
    <Card>
      <CardHeader>
        ...
        <div className="flex items-center gap-2">
          <Badge tone={summary?.readiness.ready ? "emerald" : "amber"}>{summary?.readiness.ready ? "Write enabled" : "Write blocked"}</Badge>
          <Button variant="outline" size="sm" onClick={testReadiness} disabled={!!busy || !summary?.readiness.ready}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div><Label required>Idempotency source reference</Label><Input value={sourceReference} onChange={(event) => setSourceReference(event.target.value)} placeholder="manual:stable-reference" /></div>
          </div>

          {updateTarget && <div className="grid gap-3 md:grid-cols-2">
            <div><Label>ServiceNow sys_id</Label><Input value={fields.sysId} onChange={(event) => setField("sysId", event.target.value)} placeholder="Preferred when known" /></div>
            <div><Label>ServiceNow number</Label><Input value={fields.number} onChange={(event) => setField("number", event.target.value)} placeholder="Resolved server-side when sys_id is absent" /></div>
          </div>}
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button onClick={createValidatedCommand} disabled={!!busy || !sourceReference}>
        ...
        <CardContent className="space-y-4">
          {!selected?.normalizedPreview ? <div className="rounded-xl border border-dashed border-sky-200 p-10 text-center text-[11px] text-slate-400 dark:border-slate-700">Validate a command to preview mapped field names. Long text values are never echoed here.</div> : <>
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <Button onClick={() => setConfirmExecute(selected)} disabled={!!busy || !summary?.readiness.ready || !["validated", "dry_run_ready"].includes(selected.status)}>
                <Play size={14} />Execute live
              </Button>
              {["failed", "retry_scheduled"].includes(selected.status) && <Button variant="outline" onClick={() => commandAction(selected, "retry")} disabled={!!busy || selected.attemptCount >= selected.maxAttempts}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {commands.items.map((command) => <button key={command.id} type="button" onClick={() => openCommand(command.id)} className="grid w-full gap-2 rounded-xl border border-sky-100/80 bg-white/45 p-3 text-left text-[11px] transition hover:border-sky-300 hover:bg-sky-50/50 dark:border-slate-700 dark:bg-slate-900/45 md:grid-cols-[1.3fr_1fr_1fr_130px_auto] md:items-center">
            ...
            <div><span className="text-slate-400">Source</span><p className="mt-1 truncate">{command.sourceReference}</p></div>
      ...
      <DialogContent title="ServiceNow write command" description="Safe command state, summaries, and bounded attempt history. Stored raw payloads are not returned." className="max-w-4xl">
        {selected && <div className="space-y-4 text-[11px]">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            {["validated", "dry_run_ready"].includes(selected.status) && <Button onClick={() => { setDetailOpen(false); setConfirmExecute(selected); }} disabled={!!busy || !summary?.readiness.ready}><Play size={14} />Execute live</Button>}
            {["failed", "retry_scheduled"].includes(selected.status) && <Button onClick={() => commandAction(selected, "retry")} disabled={!!busy || selected.attemptCount >= selected.maxAttempts}><RotateCcw size={14} />Manual retry</Button>}
          </div>
        </div>}
      </DialogContent>
    </Dialog>

    <Dialog open={Boolean(confirmExecute)} onOpenChange={(open) => !open && setConfirmExecute(undefined)}>
      <DialogContent title="Execute ServiceNow write?" description="This is a live provider mutation. The validated command, attempt, and safe summaries will be retained.">
        {confirmExecute && <div className="space-y-4 text-[11px]">
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-amber-800 dark:border-amber-900 dark:bg-amber-950/30">
            <p className="font-semibold">{confirmExecute.commandType.replaceAll("_", " ")}</p>
            <p className="mt-1">Source: {confirmExecute.sourceReference}</p>
            <p className="mt-1">Target: {confirmExecute.targetNumber || confirmExecute.targetSysId || "New ServiceNow Incident"}</p>
          </div>
          <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setConfirmExecute(undefined)}>Cancel</Button><Button onClick={() => commandAction(confirmExecute, "execute")}><Play size={14} />Confirm live write</Button></div>
        </div>}
      </DialogContent>
    </Dialog>
  </div>;
