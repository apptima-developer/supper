...
export function parseServiceNowSyncConfig(env: Record<string, string | undefined>): ServiceNowSyncConfig {
  return syncConfigSchema.parse({
    enabled: env.SERVICENOW_SYNC_ENABLED === "true",
    initialLookbackDays: env.SERVICENOW_SYNC_INITIAL_LOOKBACK_DAYS || 30,
    overlapSeconds: env.SERVICENOW_SYNC_OVERLAP_SECONDS || 120,
    maxRecords: env.SERVICENOW_SYNC_MAX_RECORDS || 1_000,
    maxPages: env.SERVICENOW_SYNC_MAX_PAGES || 20,
    lockTtlSeconds: env.SERVICENOW_SYNC_LOCK_TTL_SECONDS || 300,
  });
}
