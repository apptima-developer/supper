...
## AI-2.0.7 controlled writes

AI-2.0.7 corrects the separate administrator-operated [Controlled ServiceNow Write Kernel](./servicenow-write-kernel.md). It does not change the diagnostic, synchronization, or customer-mapping read paths. Live mutation is disabled independently by default.
...
Manual command transport uses a short-lived signed operation identity so a lost HTTP response can be retried without creating a second command or marker. Full semantic command material and normalized provider mutation use separate SQL-verified hashes. Exact number, marker, and `sys_id` reads verify the returned key and same-row identity before use. Every valid POST identity is appended to a per-Attempt candidate event ledger, not stored as a confirmed target or mutable command field. Reconciliation and its confirmation reference the current unresolved event; terminal-only newest resolution projection prevents inconclusive history from reopening candidate A, and a later retry appends candidate B. Create success requires either verified candidate proof (G1) or exact-marker GET recovery (G2) with a matching strict request/response identity pair, safe 2xx status, explicit exact-marker proof, no provider write, no candidate, and no prior mutation evidence in the Attempt. Update and journal success cannot replace the command's original normalized target, but may add its missing counterpart. Candidate not-applied requires a separate duplicate-risk acknowledgment. Reconciliation uses the shared nine-value evidence enum and an SQL-enforced action matrix; `provider_inconclusive` remains unresolved and journal review uses only `journal_manual_verification`. Connection testing remains an authenticated, bounded GET and is available while live writes are disabled. A fresh durable proof matching the non-secret configuration fingerprint is required before execute/retry can consume an attempt. Execute, retry, recovery, and reconciliation require an expiring one-time server confirmation tied to command version, normalized hash, and current candidate when present; database time is final authority. Stuck recovery is available only after the immutable database recovery lease (connection timeout plus safety grace). Its action sends no provider request, but the original mutation outcome remains unknown; a late provider response cannot rewrite recovered state.
