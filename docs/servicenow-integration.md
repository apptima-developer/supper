...
## AI-2.0.9 controlled writes

AI-2.0.9 corrects the separate administrator-operated [Controlled ServiceNow Write Kernel](./servicenow-write-kernel.md). It does not change the diagnostic, synchronization, or customer-mapping read paths. Live mutation is disabled independently by default.
...
Manual command transport uses a short-lived signed operation identity so a lost HTTP response can be retried without creating a second command or marker. Full semantic command material and normalized provider mutation use separate SQL-verified hashes. Exact number, marker, and `sys_id` reads verify the returned key and same-row identity before use. G1 and G2 bind lookup and verified SHA-256 marker hashes to the command-owned marker; successful create reconciliation requires the same exact marker proof. PostgreSQL validators reject missing, JSON-null, wrong-type, empty, malformed, mismatched, or extra proof material before command or Ticket-link mutation. Every valid POST identity is appended to a per-Attempt candidate event ledger, not stored as a confirmed target or mutable command field. Terminal resolution is queried separately from bounded display history, so later nonterminal events cannot reopen candidate A and a later retry safely appends candidate B. Successful non-create commands require exact PATCH endpoint, target pair, normalized field list, safe 2xx response, and strict response keys. Because authorization is acquired before every provider request and a token inside the 30-second margin is not reused, OAuth reserves one token timeout per provider request; the database recovery lease covers all provider and token requests plus two-minute grace. Late success and all late failure classes share one bounded closure after recovery and cannot mutate state.
