...
AI-1.3.1 makes that bridge derive canonical hashes from the actual normalized record material rather than an idempotency key or bodies alone. Message material includes provider/channel identity, external message and thread IDs, sender, direction, subject-bearing conversation material, normalized bodies, provider timestamp, and deterministic attachment summaries. Event material also includes the identity, conversation, external event, and requested session initialization. Changing subject, sender, direction, attachment material, or other replay-significant content therefore changes the corresponding hash. Hashing remains a pure in-memory operation with no repository, provider, Ticket, outbox, or network side effect.
