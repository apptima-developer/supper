...
Mutation delivery certainty is part of the boundary. A new create sends at most one POST and is successful only when G1 exact post-write proof matches its per-Attempt candidate event, or when G2 pre-mutation exact-marker recovery proves a GET with no provider write and no prior mutation evidence. The proof paths cannot be mixed. Valid POST pairs append immutable bounded candidate events even when proof fails; they are not confirmed targets. Once POST/PATCH dispatch begins, ambiguous mutation or proof failures become `may_have_committed` and enter `reconciliation_required`; they never enter the retry path. Retry requires a resolved current candidate, definitive `safe_to_retry`, and fresh GET readiness proof. Confirmations and reconciliation are scoped to the current candidate event; stale IDs or versions conflict. Live execution and retry require valid provider configuration; ledger reads, confirmations, manual decisions, and stuck-Attempt recovery do not construct an adapter. Recovery issues no provider request and appends immutable history. Verified reconciliation is GET-only and stores only safe evidence.
