...
Mutation delivery certainty is part of the boundary. Once POST/PATCH dispatch begins, ambiguous failures become `may_have_committed` and enter `reconciliation_required`; they never enter the retry path. Retry requires a definitive `safe_to_retry` result plus a fresh successful GET proof matching the current non-secret configuration fingerprint. Database time controls freshness and retry availability. Live execution and retry require valid provider configuration; ledger reads, confirmations, and explicit provider-unavailable recovery do not construct an adapter. Verified reconciliation is GET-only, enforces negative-evidence classifications, requires an exact target pair for success, and stores only safe evidence.
