...
Mutation delivery certainty is part of the boundary. A new create sends at most one POST and is successful only when G1 exact post-write proof matches its per-Attempt candidate event, or when G2 pre-mutation exact-marker recovery proves a strict Incident GET request and matching response `sys_id`/number pair with exact-marker proof, no provider write, no candidate, and no prior mutation evidence in that Attempt. The proof paths cannot be mixed. Valid POST pairs append immutable bounded candidate events even when proof fails; they are not confirmed targets. Terminal-only newest resolution projection ensures inconclusive history cannot reopen a resolved candidate. Once POST/PATCH dispatch begins, ambiguous mutation or proof failures become `may_have_committed` and enter `reconciliation_required`; they never enter the retry path. Retry requires a resolved current candidate, definitive `safe_to_retry`, and fresh GET readiness proof. Confirmations and reconciliation are scoped to the current candidate event; stale IDs or versions conflict. Non-create success preserves the original normalized `sys_id` or number and may add only the missing counterpart. Live execution and retry require valid provider configuration; ledger reads, confirmations, manual decisions, and eligible stuck-Attempt recovery do not construct an adapter. Recovery eligibility is owned by database time and starts after timeout plus safety grace. The recovery action issues no provider request, does not characterize the original mutation as absent, and appends immutable history; a late response cannot overwrite that state. Verified reconciliation is GET-only and stores only safe evidence.
