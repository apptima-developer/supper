...
Mutation delivery certainty is part of the boundary. A new create sends at most one POST and is successful only after a second exact correlation-marker GET proves the same marker, `sys_id`, and number. A valid POST pair is preserved as immutable bounded candidate evidence even when proof fails; it is not a confirmed target. Once POST/PATCH dispatch begins, ambiguous mutation or proof failures become `may_have_committed` and enter `reconciliation_required`; they never enter the retry path. Retry requires a definitive `safe_to_retry` result plus a fresh successful GET proof matching the current non-secret configuration fingerprint. Database time controls freshness and retry availability. Live execution and retry require valid provider configuration; ledger reads, confirmations, and candidate-matched provider-unavailable recovery do not construct an adapter. Verified reconciliation is GET-only, uses the shared evidence enum, rejects contradictory result/evidence/candidate pairs in TypeScript and SQL, requires an exact candidate match for create success, leaves inconclusive evidence unresolved, and stores only safe evidence. A conflicting target creates no Ticket link; create candidate not-applied requires a separate duplicate-risk acknowledgment.
