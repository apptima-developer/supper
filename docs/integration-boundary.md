...
Mutation delivery certainty is part of the boundary. A new create sends at most one POST and is successful only after a second exact correlation-marker GET proves the same marker, `sys_id`, and number. Once POST/PATCH dispatch begins, ambiguous mutation or proof failures become `may_have_committed` and enter `reconciliation_required`; they never enter the retry path. Retry requires a definitive `safe_to_retry` result plus a fresh successful GET proof matching the current non-secret configuration fingerprint. Database time controls freshness and retry availability. Live execution and retry require valid provider configuration; ledger reads, confirmations, and explicit provider-unavailable recovery do not construct an adapter. Verified reconciliation is GET-only, uses the shared evidence enum, rejects contradictory result/evidence pairs in TypeScript and SQL, requires an exact target pair for success, leaves inconclusive evidence unresolved, and stores only safe evidence.
