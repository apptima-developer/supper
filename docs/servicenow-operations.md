...
AI-2.0.7 corrects the separate protected **Write controls** page at `/settings/integrations/servicenow/write`. The read operations documented here remain read-only and unchanged. Live write execution is disabled by default and is documented in [Controlled ServiceNow Write Kernel](servicenow-write-kernel.md).

Write controls distinguish configuration, untested/expired/tested GET proof, live-write switch, and complete live readiness. Manual command submissions retain one server-issued operation identity until success, allowing safe lost-response replay only when the full semantic command material matches. Command detail shows the current unresolved candidate, immutable newest-first candidate history, terminal resolution status, proof status, and recovery history separately from the confirmed target; it never displays a raw provider response. Candidate-aware manual success is prefilled and locked to the current persisted pair; the page refreshes command version/hash/candidate before confirmation so a stale candidate ID cannot submit. Marking a create candidate not applied shows a high-risk duplicate warning and requires its own acknowledgment. Retry is visible only for `retry_scheduled` commands with `retryAllowed=true` while live writes are ready. `reconciliation_required` commands instead open a verified-result dialog. A stuck `executing` Attempt displays its start and database-derived recovery-eligible timestamps; recovery stays disabled until the connection timeout plus safety grace has elapsed. The recovery action makes no provider request, the original mutation outcome remains unknown, and a late response is rejected without changing the ledger. Reconciliation itself performs no provider mutation.
