...
AI-2.0.6 ServiceNow Write is relational-only. Its command, Attempt, append-only per-Attempt candidate event, mapping, connection, short-lived readiness proof, Ticket link, immutable reconciliation, and immutable stuck-Attempt recovery ledgers use dedicated relational tables. `service_role` has read-only table grants; all mutations use validated service-role-only RPCs. Attempt finish is the only candidate writer, is idempotent only for identical terminal material, and appends a later candidate only after the current one is resolved. Reconciliation references the exact candidate event it resolves. SQL separately verifies semantic command identity, provider normalization, create proof, candidate continuity, and action/evidence rules. Database time owns observation, proof, confirmation, retry, recovery, and reconciliation chronology. Ledger recovery does not require provider configuration and never performs a provider request; live mutation still does. The subsystem never stores commands in `app_store` or local JSON and never falls back to the Vercel filesystem.
