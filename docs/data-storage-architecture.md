...
AI-2.0.7 ServiceNow Write is relational-only. Its command, Attempt, append-only per-Attempt candidate event, mapping, connection, short-lived readiness proof, Ticket link, immutable reconciliation, and immutable stuck-Attempt recovery ledgers use dedicated relational tables. `service_role` has read-only table grants; all mutations use validated service-role-only RPCs. Attempt finish is the only candidate writer, is idempotent only for identical terminal material, and appends a later candidate only after the current one is terminally resolved. Candidate detail selects the newest terminal resolution per event and cannot be overwritten by nonterminal observations. SQL separately verifies semantic command identity, provider normalization, exact G1/G2 create proof, original non-create target continuity, candidate continuity, and action/evidence rules. Database time owns observation, proof, confirmation, retry, recovery eligibility, and reconciliation chronology; immutable `recoverable_at` includes connection timeout plus bounded safety grace. Ledger recovery does not require provider configuration and its action never performs a provider request, while truthfully retaining the original mutation outcome as unknown. Late responses cannot rewrite recovered state. The subsystem never stores commands in `app_store` or local JSON and never falls back to the Vercel filesystem.
