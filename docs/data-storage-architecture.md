...
AI-2.0.9 ServiceNow Write is relational-only. Its command, Attempt, append-only per-Attempt candidate event, mapping, connection, short-lived readiness proof, Ticket link, immutable reconciliation, and immutable stuck-Attempt recovery ledgers use dedicated relational tables. Attempt rows persist the immutable worst-case provider-plus-OAuth request count, recovery milliseconds, and eligibility timestamp derived from committed database material. SQL rejects JSON-null or wrong-type proof values and independently validates marker hashes for G1, G2, and create reconciliation plus strict provider summaries for non-create success before target or Ticket-link mutation. Candidate detail obtains terminal resolution through a separate unbounded-by-display query; the newest terminal result remains authoritative after any number of later nonterminal display events. One shared late-outcome closure preserves recovered command, Attempt, link, candidate, and recovery state. The subsystem never stores commands in `app_store` or local JSON and never falls back to the Vercel filesystem.
