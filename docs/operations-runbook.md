...
AI-2.0.7 corrects optional ServiceNow write controls. `SERVICENOW_WRITE_ENABLED` defaults to false and gates only live provider mutation; the bounded GET readiness test remains available. `SERVICENOW_WRITE_MAX_ATTEMPTS` defaults to 3 and is bounded from 1 through 10. Optional `SERVICENOW_CREDENTIAL_VERSION` is non-secret rotation metadata and defaults to `unversioned`; change it whenever the configured credential changes.

An ambiguous post-dispatch outcome is an operator event, not a retry event. Create dispatches no second POST and requires an exact post-write marker GET before G1 success. Exact-marker G2 recovery is GET-only and must carry a matching strict request/response Incident identity pair, safe status, exact-marker proof, and no provider write or candidate evidence. Leave failed proof in `reconciliation_required`; the displayed identity is the current unresolved candidate event, not a confirmed target. Terminal resolution remains authoritative even when later nonterminal observations are displayed. Manual decisions and confirmations must reference the freshly loaded current event. Resolve candidate A before retry; candidate B then appends while A remains in history. A target or candidate conflict stays unresolved and creates no Ticket link. Candidate not-applied requires its separate risk acknowledgment. Retry only when the command explicitly reports `retry_scheduled`, `retryAllowed=true`, and readiness is fresh. If an Attempt is genuinely stuck in `executing`, wait until its displayed database-derived recovery eligibility, then refresh and use **Recover stuck Attempt** after its dedicated confirmation. The recovery action makes no ServiceNow request; it closes the Attempt as uncertain, records that the original mutation outcome remains unknown, moves the command to reconciliation, and appends immutable history. A late response is rejected and logged with safe identifiers only. Database time remains authoritative.
