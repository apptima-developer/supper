...
AI-2.0.9 corrects optional ServiceNow write controls. `SERVICENOW_WRITE_ENABLED` defaults to false and gates only live provider mutation; the bounded GET readiness test remains available. `SERVICENOW_WRITE_MAX_ATTEMPTS` defaults to 3 and is bounded from 1 through 10. Optional `SERVICENOW_CREDENTIAL_VERSION` is non-secret rotation metadata and defaults to `unversioned`; change it whenever the configured credential changes.

An ambiguous post-dispatch outcome is an operator event, not a retry event. G1, G2, and successful create read-back reconciliation must include lookup/verified marker hashes matching the SQL-computed command-marker hash; JSON-null, wrong-type, malformed, mismatched, or extra evidence is rejected. Non-create success must show exact PATCH endpoint, final target pair, normalized field list, and strict safe 2xx response. Terminal candidate state comes from its separate terminal projection, not the 100-row display history. Before **Recover stuck Attempt**, wait for the displayed database timestamp: basic create/number/`sys_id` commands reserve 3/2/1 request timeouts; OAuth reserves 6/4/2 because every provider request may require a fresh token; all receive two-minute grace. At `timeout_ms=60000`, the resulting minimum leases are 300/240/180 seconds for basic and 480/360/240 seconds for OAuth. A late success, provider failure, candidate-bearing proof failure, or internal adapter error is rejected once with safe identifiers/classification only and cannot alter any ledger or Ticket link.
