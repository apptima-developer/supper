...
Payloads are bounded and recursively reject normalized and contextual compact credential-like keys at every depth using the same TypeScript/PostgreSQL decision fixture as intake, target references, and delivery metadata. The classifier rejects compact token/secret/signed-URL credential concepts without treating ordinary words such as `tokenization` or `secretarial` as secrets. Canonical/idempotency payload numbers must be safe integers; fractions and unsafe integers return `INTAKE_CANONICAL_NUMBER_INVALID`. Outbox metadata accepts only `source` and `diagnostic`. Provider filters accept every integration target provider (`email`, `n8n`, `servicenow`, `internal`, `line`, `web`, and `freshservice`), unlike ordinary intake-channel filters.
