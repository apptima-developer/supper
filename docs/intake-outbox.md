...
`support_enqueue_integration_outbox(jsonb)` starts new commands as `pending` with zero attempts. Reusing an idempotency key with identical normalized material returns `unchanged` with the command's real bounded status and attempt count; it never resets or requeues a `processing`, `retrying`, `succeeded`, `dead_letter`, or `cancelled` command. Changed material returns `INTEGRATION_OUTBOX_IDEMPOTENCY_CONFLICT`.

Payloads are bounded and recursively reject normalized credential-like keys at every depth. Outbox metadata accepts only `source` and `diagnostic`. Provider filters accept every integration target provider (`email`, `n8n`, `servicenow`, `internal`, `line`, `web`, and `freshservice`), unlike ordinary intake-channel filters.
