...
AI-2.0.8 is an administrator-operated, server-only write boundary for ServiceNow Incidents. It requires `DATA_BACKEND=supabase-relational`, reuses server-only ServiceNow credentials, and keeps live provider mutation disabled until `SERVICENOW_WRITE_ENABLED=true`.
...
Ledger recovery is provider-independent. At Attempt begin, SQL derives and persists immutable `provider_request_budget`, `recovery_budget_ms`, and `recoverable_at` from database time and committed command/connection material. Create reserves three provider timeouts, number-target non-create reserves two, and `sys_id`-target non-create reserves one; OAuth client-credential mode adds one token-acquisition allowance. The full request budget then receives two minutes of safety grace and is capped at 15 minutes. Neither the browser nor the service supplies these values. Confirmation issuance and recovery reject before the complete lease, including at a theoretical final HTTP timeout before grace; the UI shows the safe eligible timestamp/countdown. Recovery evidence states only `recoveredByAdministrator=true`, `recoveryOperationProviderRequestPerformed=false`, and `originalMutationOutcome=unknown`.
...
15. With `timeout_ms=60000`, leave disposable create, number-target, and `sys_id`-target Attempts in `executing`. Confirm their basic-auth recovery budgets are at least 300000, 240000, and 180000 milliseconds respectively, OAuth adds one token-acquisition timeout, and neither the UI nor a direct request permits recovery at the final provider-timeout boundary before grace. At database eligibility, refresh and recover one; confirm no provider request, unknown original outcome, `reconciliation_required`, and immutable recovery history.
...
17. For G2, verify a valid exact-marker GET pair succeeds only when request and response marker hashes both equal the SQL-computed full command-marker hash. Missing request hash, missing response hash, another marker hash, request/response mismatch, forged flags, missing identity, mismatched target pair, and arbitrary request shape must fail without a Candidate or link. For update, comment, and work note, verify exact PATCH table/endpoint, final pair, sorted normalized field set, strict 2xx response, and optional bounded state; empty evidence, wrong method/endpoint/pair, extra response keys, and crossed journal fields must fail without a link. Successful update reconciliation must likewise carry exact GET endpoint, pair, and complete field-match proof.
