...
## Applying AI-2.0.5 ServiceNow write kernel
...
The migration records version `202607230001`, creates eight RLS-protected write-kernel tables including short-lived readiness proofs and append-only mutation candidates, and exposes validated configuration, mapping, readiness, command, confirmation, attempt, and reconciliation RPCs. AI-2.0.5 adds immutable candidate continuity on top of the semantic command hash, exact provider proof, database-clock authority, explicit evidence persistence, and the SQL action matrix. It was amended in place only because explicit delivery history proves this version has never been remotely applied. `service_role` can select ledger rows but cannot directly insert, update, or delete them. It contains no credentials, does not alter existing intake or read-side synchronization rows, and performs no ServiceNow request.
...
4. Confirm version `202607230001`, RLS on all eight write tables, select-only table grants for `service_role`, append-only candidate enforcement, and privileged RPC execution only for `service_role`, including `support_record_servicenow_write_readiness(jsonb)`.
