...
## Applying AI-2.0.6 ServiceNow write kernel
...
The migration records version `202607230001`, creates eight RLS-protected `servicenow_write_*` tables plus the controlled Ticket-link table, and exposes validated configuration, mapping, readiness, command, confirmation, Attempt, recovery, and reconciliation RPCs. AI-2.0.6 replaces the single candidate shape with an append-only per-Attempt candidate event ledger, candidate-scoped reconciliation, atomic terminal idempotency, strict JSON proof checks, exact-marker no-mutation recovery, and immutable stuck-Attempt recovery history. It was amended in place only because explicit delivery history proves this version has never been remotely applied. `service_role` can select ledger rows but cannot directly mutate them. The migration contains no credentials, does not alter existing intake or read-side synchronization rows, and performs no ServiceNow request.
