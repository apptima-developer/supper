...
## Applying AI-2.0.7 ServiceNow write kernel
...
The migration records version `202607230001`, creates eight RLS-protected `servicenow_write_*` tables plus the controlled Ticket-link table, and exposes validated configuration, mapping, readiness, command, confirmation, Attempt, recovery, and reconciliation RPCs. AI-2.0.7 adds a database-owned recovery lease derived from connection timeout plus bounded grace, truthful recovery evidence, late-finish rejection, strict exact-pair G2 proof, original non-create target continuity, and terminal-only candidate projection to the append-only per-Attempt candidate event design. It was amended in place only because explicit delivery history proves this version has never been remotely applied and this correction forbids remote application before review. `service_role` can select ledger rows but cannot directly mutate them. The migration contains no credentials, does not alter existing intake or read-side synchronization rows, and performs no ServiceNow request.
