...
`support_accept_intake_event_v3(jsonb)` validates a normalized server payload and commits the event, identity, conversation, message, attachment metadata, delivery ledger row, and optional session atomically. The legacy and v2 entry points delegate to the same private implementation for rolling compatibility. The service derives canonical SHA-256 hashes; callers may not choose trusted hash material. SQL recomputes the same bounded material before accepting it, then reconstructs the persisted Message and verifies its stored hash before commit.
...
Hosted Supabase commonly installs `pgcrypto` in the `extensions` schema. Intake migrations do not assume it is in `public`: migration 003 owns one internal SHA-256 helper with a controlled `pg_catalog, public, extensions, pg_temp` search path, and migration 004 applies that same search path to the preserved migration-002 write implementation after renaming it. No extension relocation, ownership change, or compatibility wrapper is required. Migrations 003 and 004 were amended only because the isolated target had committed 001 and 002 while the failed 003 transaction and all of 004 remained unapplied; applied migrations 001 and 002 were not changed.

Exact event redelivery keeps `processing_status=accepted`, sets `redelivery=true`, and increments lifetime counters; it also appends one metadata-only row to `intake_event_deliveries` using that request's `receivedAt`, request ID, correlation ID, delivery number, and delivery type. Operations 24-hour accepted, duplicate, and failed metrics use this chronological ledger rather than lifetime counters, so an old duplicate is not attributed to a recent delivery. Ledger rows contain identifiers, sequence, type, timestamps, and recursively credential-safe metadata only, never payloads, message bodies, external subjects, or credentials. Same-ID changed material returns a bounded replay conflict and rolls the transaction back. An exact message replay reuses its original identity and conversation and creates no content or session rows. A conflicting message conversation, sender, direction, type, reply target, content, provider time, or attachment returns `INTAKE_MESSAGE_REPLAY_MISMATCH`. Ordinary message activity never reopens a closed or archived conversation.

Concurrency is scoped to `(channel, external event)`, message, conversation, subject, and attachment identities using transaction advisory locks plus row locks only for touched records. Every public RPC generation uses the same order: enabled-channel lookup without an exclusive lock, Event advisory/row, Message advisory/row, Conversation advisory/row, Identity advisory/row, then Attachment advisory/row locks sorted by external attachment ID. The enabled-channel lookup is not locked, so unrelated events in one channel can proceed concurrently.
...
One recursive sensitive-key policy is enforced in TypeScript and SQL from one committed decision fixture. Keys are split across camel/Pascal case, normalized across separators, lowercased, and also compacted to ASCII alphanumerics before classification. Contextual compact forms cover channel/app/API tokens, token values, app/OAuth/signing/client secrets, signed or presigned asset URLs, bearer credentials, and authorization values. Matching is bounded rather than an unrestricted `token` or `secret` substring search, so ordinary words such as `tokenizer`, `tokenization`, `secretariat`, `secretarial`, `monkey`, and `keyboard` remain valid. Credential-bearing input returns `INTAKE_SENSITIVE_DATA_REJECTED`. Non-ASCII and control-character key tricks, prototype keys, cycles, accessors, raw provider payloads, and nested occurrences in arrays are rejected. Metadata uses strict per-record allowlists for channels, events, identities, conversations, messages, attachments, sessions, bindings, deliveries, and outbox commands; unknown keys are invalid.
...
After an authorized operator applies migrations in order through `202607220004_unified_intake_core_final_integrity.sql` to the verified isolated `supper-ai-dev` project:
