-- SUPPER Phase 0.2.1 hotfix: make table-column precedence explicit inside the
-- PL/pgSQL RPC because RETURNS TABLE creates output variables with matching names.
...
comment on function public.support_record_login_failure(
  ...
  integer,
  integer
) is 'Server-only atomic login failure recorder. Table-column precedence is explicit for PL/pgSQL output-variable safety.';

insert into public.support_schema_migrations (version, description, checksum, applied_by)
values ('202607180002', 'Fix login rate-limit RPC PL/pgSQL variable conflict', null, current_user)
